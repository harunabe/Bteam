...
$(function() {
  ...
  setTimeout(function(){
    window.location.href = 'miss.html';},60 * 1000); //時間制限

  //Timer
  const totalTime = 60000;  //時間表示
  ...
  $('.piece').click(function() {
    ...
    if (sel == no) {
      ...
      $(this).animate({left: getx(pos[no]), top: gety(pos[no])}, function() {
        // アニメーション終了時にクリア判定する。
        var clear = true;
        for (var i = 0; i < 25; i++) if (pos[i] != i) clear = false;
...
// ピースの番号から座標を得る。
function getx(n) {return (n % 5) * 100;}
function gety(n) {return Math.floor(n / 5) * 100;}
